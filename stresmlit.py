...
access_token = st.sidebar.text_input("GitLab Access Token", type="password")
...
def get_open_merge_requests(gitlab_url, access_token, project_id):
...
def get_merge_request_changes(gitlab_url, access_token, project_id, mr_id):
...
def post_comment_on_merge_request(gitlab_url, access_token, project_id, mr_id, comment):
...
if access_token and project_id:
    ...
    merge_requests = get_open_merge_requests(gitlab_url, access_token, project_id)
    ...
    if merge_requests:
        ...
        if selected_mr:
            ...
            changes = get_merge_request_changes(
                gitlab_url, access_token, project_id, selected_mr["id"]
            )
                    ...
                    with st.spinner("Reviewing merge request with ChatGPT..."):
                        ...
                        if st.button("Post Comment to GitLab"):
                            post_comment_on_merge_request(
                                gitlab_url, access_token, project_id, selected_mr["id"], feedback
                            )
            else:
