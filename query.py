...
SELECT
    ...
    SUM(CASE WHEN t.isActive = 1 THEN 1 ELSE 0 END) AS tickets_still_open,
    SUM(CASE WHEN t.isActive = 0 THEN 1 ELSE 0 END) AS tickets_closed,
    COALESCE(ct.total_closed, 0) AS total_closed_on_day
FROM 
    TicketData t
LEFT JOIN (
    SELECT
...
ON t.request_day = ct.close_day
GROUP BY 
    t.request_day, ct.total_closed
ORDER BY 
    t.request_day;
